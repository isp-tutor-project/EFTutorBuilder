...
const ANDOMDOCUMENT:string  = "DOMDocument.xml";
const EFLOADERCONFIG:string = "EFconfig.json";
...
function generateModuleGUID() {
    ...
    let domDocPath:string = path.join(twd, module_name, ANDOMDOCUMENT);    
    ...
    // 
    let _document:string = fs.readFileSync(domDocPath, "utf8");
    ...
    let loaderPath:string = path.join(twd, module_name, EFLOADERCONFIG);
    ...
    // Generate the new DOMDocument from the XML
    // 
    let loaderUpdate:string = JSON.stringify(loaderDoc, null, '\t');
