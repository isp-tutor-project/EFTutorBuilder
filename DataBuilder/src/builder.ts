...
const DATA_BUILD:string     = "EFbuild";
const IMG_PATH:string       = "images";
const RELIMG_PATH:string    = "images/";
const DATA_GENFILE:string   = "_EFTUTORDATA.json";
const LIBR_GENFILE:string   = "_EFLIBRARYDATA.json";
const LIBR_SRCFILE:string   = "LIBRARY.json";
const LIBRARY:string        = "_LIBRARY";
...
const GLOBALTHIS:string  = "GLOBALTHIS";
...
let libBuildPath:string;

let twd:string;
let rwd:string;
let modName:RegExpExecArray;
let tutName:RegExpExecArray;

let lModules:Array<string>      = new Array<string>();
let ldataModules:Array<string>  = new Array<string>();
...
function processDataFiles(moduleName:string, fileList:Array<string>) : any {
    ...
    try {
        ...
        for(let file of fileList) {
            ...
            if(data[IMPORT_SIGNATURE]) {
                ...
                data[IMPORT_SIGNATURE].forEach((importrequest:string) => {

                    let path:Array<string>  = importrequest.split("|");                   
                    let files:Array<string> = path[1].split(",");                   
...
function mergeDataObjects(data_assets:any, data:any, parent:string) {
...
function isAnObject(obj:any) : boolean
{
    if(obj==null) return false;
    return obj.constructor.name.toLowerCase() === "object"
}

function processLibraryDependencies(library:any, libraryTar:any) : any {
            ...
            if(Array.isArray(controlSpec)) {
                ...
                controlSpec.forEach((controlItem , Index) => {
                    ...
                    if(controlItem.src && controlItem.src.startsWith(EXTERN_SIGNATURE)) {
                        ...
                        let pathArray:Array<string> = controlItem.src.split(".");
                ...
                if(controlSpec.src && controlSpec.src.startsWith(EXTERN_SIGNATURE)) {
                    ...
                    let pathArray:Array<string> = controlSpec.src.split(".");
...
function processExternalDep(src:string, controlName:string, controlClass:string, libTargetElement:any, controlType:string) {

    let externPath= path.join(twd, module_name, DATA_PATH, src + HTML_EXT);

    let htmlData:string = fs.readFileSync(externPath, "utf8");
...
function updateProcessedData(data:any, path:string) {
...
function listDataFiles() : Array<string>{

    let fpath:string = path.join(twd, module_name, DATA_PATH); 
